...
inline uint64_t gva_to_gpa(cr3_t guest_cr3, uint64_t address, uint64_t& offset_to_next_page) {
	...
	auto pdpte_1gb = reinterpret_cast<pdpte_1gb_t*>(base + (pml4e.page_pa << 12))[va.pdpt_index];
	...
	if (pdpte_1gb.large_page) {
		...
		return (pdpte_1gb.page_pa << (12 + 9 + 9)) + offset;
	}

	auto pde_2mb = reinterpret_cast<pde_2mb_t*>(base + (pdpte.page_pa << 12))[va.pd_index];
	...
	if (pde_2mb.large_page) {
		...
		return (pde_2mb.page_pa << (12 + 9)) + offset;
	}
